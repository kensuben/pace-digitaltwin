...
export function RackDesigner({ scenarioId, isLocked, racks: initialRacks, unplacedDevices: initialUnplaced }: {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid items-start gap-5 xl:grid-cols-[minmax(220px,0.65fr)_minmax(320px,1fr)_minmax(300px,0.9fr)]">
      ...
      <section className="grid min-w-0 gap-6">{racks.map((rack) => <RackCabinet key={rack.id} rack={rack} selected={selected} disabled={isLocked || isPending} onManageVms={setVmServerId} onPlace={placeAt} onSelect={setSelectedId} onRemove={(device) => void persist(device, null)}/>)}
      ...
      <div className={selected ? "fixed inset-x-3 bottom-3 z-30 max-h-[65dvh] overflow-y-auto rounded-2xl shadow-2xl xl:sticky xl:inset-auto xl:top-28 xl:z-auto xl:max-h-none xl:min-w-0 xl:overflow-visible xl:shadow-none" : "min-w-0"}>
        {selected ? <DeviceDetailsPanel key={`${scenarioId}:${selected.id}`} deviceId={selected.id} scenarioId={scenarioId}
          rackLabel={selected.rackId ? `${racks.find((rack) => rack.id === selected.rackId)?.code ?? "Rack"} · U${selected.rackUnitStart}` : "Chờ lắp lên rack"}
          virtualMachines={selected.virtualMachines} onClose={() => setSelectedId(null)} onManageVms={() => setVmServerId(selected.id)}/>
          : <aside className="rounded-2xl border border-dashed bg-card p-5 text-sm text-muted-foreground"><h2 className="mb-2 font-bold text-foreground">Thông tin thiết bị</h2>Chọn một thiết bị trên rack hoặc trong danh sách chờ để xem thông tin và cấu hình.</aside>}
      </div>
    </div>
...
function RackCabinet({ rack, selected, disabled, onManageVms, onPlace, onSelect, onRemove }: {
  ...
  return <div className="overflow-hidden rounded-2xl border-4 border-slate-700 bg-slate-950 shadow-2xl">
    ...
    <div className="relative bg-slate-900/80" style={{ height: rack.rackUnits * unitHeight }}>
      ...
      {rack.devices.map((device) => { if (!device.rackUnitStart) return null; const top = (rack.rackUnits - (device.rackUnitStart + device.rackUnits - 1)) * unitHeight; return <div className={`absolute left-11 right-2 z-10 flex items-center gap-2 overflow-hidden rounded border px-2 text-white shadow-lg ${selected?.id === device.id ? "ring-2 ring-cyan-300 border-cyan-200" : ""} ${device.category === "SERVER" ? "border-violet-300/60 bg-gradient-to-r from-violet-950 to-slate-800" : "border-cyan-300/50 bg-gradient-to-r from-cyan-950 to-slate-800"}`} draggable={!disabled} key={device.id} onClick={() => onSelect(device.id)} onDragStart={(event) => event.dataTransfer.setData("application/x-rack-device", JSON.stringify(device))} style={{ height: device.rackUnits * unitHeight - 2, top: top + 1 }}><GripVertical className={`shrink-0 ${device.category === "SERVER" ? "text-violet-300" : "text-cyan-400"}`} size={14}/><button type="button" aria-label={`Xem thông tin ${device.hostname}`} aria-pressed={selected?.id === device.id} className="min-w-0 flex-1 text-left focus-visible:outline-2 focus-visible:outline-cyan-300"><p className="truncate font-mono text-xs font-bold text-cyan-100">{device.hostname}</p><p className="truncate text-[10px] text-slate-400">{device.sku} · {device.rackUnits}U</p></button>{device.category === "SERVER" && <button aria-label={`Quản lý Virtual Machines của ${device.hostname}`} className="flex shrink-0 items-center gap-1 rounded bg-violet-400/15 px-1.5 py-1 text-[9px] font-bold text-violet-200 hover:bg-violet-400/25" onClick={(event) => { event.stopPropagation(); onManageVms(device.id); }} title="Quản lý Virtual Machines" type="button"><Cpu size={11}/>{device.virtualMachines.length} VM</button>}{!disabled && <button aria-label={`Đưa ${device.hostname} ra khỏi rack`} className="rounded p-1 text-slate-400 hover:bg-white/10 hover:text-white" onClick={(event) => { event.stopPropagation(); onRemove(device); }} title="Đưa ra khỏi rack" type="button"><RotateCcw size={13}/></button>}</div>; })}
    </div>
